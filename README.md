# awesome-onec-neurofish
Дружим 1С и большие языковые модели. Подборка полезного материала по теме.

[Чат для обсуждений](https://t.me/onec_neurofish)

# Полезные ссылки

1. https://help.openai.com/en/articles/6654000-best-practices-for-prompt-engineering-with-the-openai-api - промпт-гайд от OpenAI
2. https://github.com/DenisSergeevitch/chatgpt-custom-instructions - системный промпт для ChatGPT
3. https://cookbook.openai.com/
4. https://platform.openai.com/chat/edit?optimize=true - оптимизатор промптов

## Интересные репозитории

1. https://github.com/alkoleft/amusement-park-vibe - репозиторий с материалами доклада BIA Tech. Рекомендуется начать изучение вайбкодинга именно с него.
2. https://github.com/FSerg/1C-SSH-Configurator-UI - UI обёртка для удалённой работы с агентом конфигуратора 1С через SSH.
3. https://takopi.dev/ - общаяемся со своим ИИ-агентом через Telegram.

## Официальные гайды для GPT-5

<!--by https://t.me/onec_neurofish/1223 -->
1. https://cdn.openai.com/API/docs/gpt-5-for-coding-cheatsheet.pdf - GPT-5 для программирования
2. https://cookbook.openai.com/examples/gpt-5/gpt-5_prompting_guide - GPT-5 гайд на промпты

# IDE-шки

1. Cursor - https://www.cursor.com/
2. Antigravity - https://antigravity.google/
3. Kiro - https://kiro.dev

## Правила

1. https://github.com/bia-technologies/yaxunit/tree/develop/.cursor/rules
2. https://github.com/bia-technologies/rat/tree/develop/.cursor/rules
3. https://github.com/comol/cursor_rules_1c

## SKILLS

1. https://github.com/FSerg/1c-llm-requests - Расширение для 1С и SKILL для запросов данных в базе 1С.
2. https://skills.sh/intellectronica/agent-skills/context7 - классный SKILL для поиска актуальной документации/справки по библиотекам/фреймворкам. Замена одноименному MCP.

## Плагины

1. Continue - https://www.continue.dev/
2. CodeGPT - https://codegpt.co/
3. RooCode - https://roocode.com/
4. Codex - https://developers.openai.com/codex/ide/

# От вендора

Напарник - https://code.1c.ai/

# MCP

1. https://github.com/artesk/1C_MCP_metadata
1. https://github.com/vladimir-kharin/1c_mcp
1. https://github.com/FSerg/mcp-1c-v1
1. https://github.com/alkoleft/mcp-bsl-context
1. https://infostart.ru/marketplace/2405549/
1. https://github.com/burlakovmb/Claude-AI-assistant-for-1C
1. https://github.com/alkoleft/mcp-onec-test-runner/

# API-провайдеры:

1. https://github.com/cheahjs/free-llm-api-resources
2. https://openrouter.ai/ - OpenRouter, любые модели, но с токен-тарификацией.
3. https://www.cerebras.ai/ - Cerebras, хостят только GLM 4.7, Qwen 3 32B/235B, GPT OSS но делают это на невероятных скоростях. Для работы дорого, но для демо с вау-эффектом - самое то.

# Прокси агентских подписок

Когда нужно свою ИИ-подписку расшарить на другие приложения и сервисы
1. https://github.com/router-for-me/CLIProxyAPIPlus - проверенное многими решение
2. https://github.com/decolua/9router
3. https://github.com/justlovemaki/AIClient-2-API